# SALT Split - детерминирование сплитование 

**SALT Split** - это модуль разделения пользователей на группы для A/B-тестирования. При техническом реализации, всегда нужно отказываться от простого рандома (или лучше вообще не реализовать такое) в пользу надежного детерминированного хэширования.

### В чем проблема обычного рандома?
Использование обычного генератора случайных чисел в реальных продуктах создает риски:
1.  **Перетекание юзеров:** Пользователь может увидеть новый дизайн с телефона, а старый — с ноутбука. Это портит пользовательский опыт, а **тесты становятся невалидны**
...
**Суть**
![Схема слоев и бакетов](a27ca4aac24448c63acd42b2d4e85ca6.png)
*(На схеме видно, как User ID проходит через разные "слои". В Test 1 он попал в Treatment, а в Test 2 — в Control. Это независимые процессы.)*
...
#### 2. Слои (Layers) — Параллельные тесты
...
* **QA и Dev слоты:** Отдельные слоты для команды. Добавлю реализацию чуть позже
...
* **Алгоритм:** `hash(user_id + layer_id + salt) % n_slots`
* **Хэш-функция:** Используем быструю функцию с равномерным распределением (например, `murmur3` или `spookyhash`, чаще всего `mda5`).
* **Slots (Бакеты):** Аудитория делится на N виртуальных слотов (обычно 100 или 1000). Группам назначаются диапазоны (например, слоты 0-49 = Control, 50-99 = Treatment).
